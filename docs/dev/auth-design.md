...
```
密码登录 ──→ UserService.Login()           ──→ issueUserToken(user)
Passkey  ──→ UserService.PasskeyLoginFinish() ──→ issueUserToken(user)
OAuth    ──→ UserService.HandleOAuthCallback() ──→ issueUserToken(user)
```
...
├─ TokenBlacklist.IsRevoked() 检查是否被吊销
...
```go
// internal/service/auth/token_blacklist.go
type TokenBlacklist struct {
    cache cache.ICache[string, any]  // Ristretto 内存缓存
}
```

- `Revoke(jti, remainTTL)` — 将 JTI 存入缓存，TTL = token 剩余有效期
- `IsRevoked(jti)` — O(1) 查找
...
| 文件 | 职责 |
|---|---|
| `internal/model/auth/scope.go` | Token 类型常量、Scope/Audience 定义 |
| `internal/model/auth/auth.go` | MyClaims、TokenPair、OAuth/Passkey DTO |
| `internal/util/jwt/jwt.go` | JWT 签发与解析（CreateClaims / CreateRefreshClaims / ParseToken / ParseRefreshToken） |
| `internal/util/cookie/cookie.go` | HttpOnly Cookie 读写工具 |
| `internal/repository/auth/auth.go` | 基于 Ristretto 的 JTI 黑名单与 OAuth code 缓存仓储 |
| `internal/service/user/token.go` | issueUserToken() — 统一的双 token 签发入口 |
| `internal/service/user/user.go` | Login / PasskeyLoginFinish / HandleOAuthCallback / ExchangeOAuthCode |
| `internal/handler/auth/auth.go` | /api/auth/refresh, /api/auth/logout, /api/auth/exchange |
| `internal/handler/user/user.go` | 密码登录 handler（写 Cookie + 返回 access_token） |
| `internal/handler/user/passkey.go` | Passkey 登录 handler |
| `internal/middleware/auth.go` | JWT 鉴权中间件（含黑名单检查） |
