  ...
  <div class="stream-root">
    <div class="stream-scroll">
      <div class="stream-track">
        <div v-for="(row, rowIdx) in imageRows" :key="rowIdx" class="stream-row">
          <div
            v-for="(cell, colIdx) in row"
            :key="getImageKey(cell.image, cell.idx)"
            class="stream-card"
            :class="{ 'stream-card--row-start': colIdx === 0 }"
...
import type { GalleryStreamProps } from './types'

/** 每行最多邮票数量 */
const STREAM_MAX_PER_ROW = 5

const props = defineProps<GalleryStreamProps>()

const imageRows = computed(() => {
  const list = props.images
  const rows: { image: App.Api.Ech0.FileObject; idx: number }[][] = []
  for (let i = 0; i < list.length; i++) {
    const rowIndex = Math.floor(i / STREAM_MAX_PER_ROW)
...
.stream-row {
  ...
  max-width: 100%;
  min-height: calc(
    var(--stamp-outer) * var(--stream-hover-scale) + var(--stamp-outer) * 0.28 + 0.75rem
  );
  padding-top: calc(var(--stamp-outer) * (var(--stream-hover-scale) - 1) * 0.38 + 8px);
  padding-bottom: calc(var(--stamp-outer) * (var(--stream-hover-scale) - 1) * 0.32 + 6px);
  padding-left: 4px;
  padding-right: 4px;
  box-sizing: border-box;
}
...
.stream-row + .stream-row {
  margin-top: calc(var(--stamp-outer) * -1 * var(--stream-row-overlap));
}

.stream-card {
  flex: 0 0 auto;
  width: var(--stamp-outer);
  margin-left: calc(var(--stamp-outer) * -1 * var(--stream-overlap));
  transform: rotate(var(--stream-rot, 0deg)) translateY(var(--stream-y, 0px));
  transform-origin: center center;
  transition: transform 0.2s ease;
}

.stream-card--row-start {
  margin-left: 0;
}
 ...
 */
:deep(.gallery-image-frame.stream-frame) {
  width: var(--stamp-outer);
  height: var(--stamp-outer);
  min-width: 0;
  min-height: 0;
...
:deep(.gallery-image-frame.stream-frame .image-skeleton) {
  border-radius: 0 !important;
}

:deep(.gallery-image-frame.stream-frame .echoimg.stream-img) {
  display: block;
  width: 100% !important;
  ...
  border-radius: 0 !important;
  border: var(--stamp-white-border) solid var(--color-gallery-stamp-border);
  box-sizing: border-box;
