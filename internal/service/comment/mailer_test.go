...
func TestResolveSMTPTransport(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			port, sslPort, policy := resolveSMTPTransport(tt.configuredPort)
			if port != tt.expectedPort {
				t.Fatalf("expected port %d, got %d", tt.expectedPort, port)
			}
			if sslPort != tt.expectedSSLPort {
				t.Fatalf("expected sslPort %v, got %v", tt.expectedSSLPort, sslPort)
			}
