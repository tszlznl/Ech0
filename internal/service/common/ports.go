...
type Service interface {
	CommonGetUserByUserId(ctx context.Context, userId uint) (userModel.User, error)
	UploadFile(userId uint, file *multipart.FileHeader, category storageDomain.Category) (commonModel.FileDto, error)
	DeleteFile(userid uint, dto commonModel.FileDeleteDto) error
	...
	GetFilePresignURL(userid uint, s3Dto *commonModel.GetPresignURLDto, method string) (commonModel.PresignDto, error)
	CleanupOrphanFiles() error
	GetWebsiteTitle(websiteURL string) (string, error)
	DeleteFileRecord(ctx context.Context, key string) error
	DeleteStoredFile(key string) error
}

type CommonRepository interface {
	GetUserByUserId(ctx context.Context, id uint) (userModel.User, error)
...
type FileRepository interface {
	...
	GetByID(ctx context.Context, id uint) (*fileModel.File, error)
