---
title: 常见问题
description: 与 README 常见问题对齐的简答
---

以下与仓库根目录 **README.zh.md → 常见问题** 对应，便于官网与仓库说法一致。

### Ech0 是什么？

轻量、开源、**自托管**的发布工具：用一条**时间线**发短文、链接和媒体，数据在你自己的服务器上，强调简洁阅读和自主可控。
...
用面板里的 **快照导出 / 迁移导入**。平时请定期备份你映射的**数据目录**（例如 Docker 的 `/opt/ech0/data`）。若启用对象存储，媒体在存储后端，库在本地或按你的部署而定。
...
后端会逐个拉取实例信息；**连不上的实例会被丢弃**，只显示成功拉取到的。
...
见仓库 **[存储迁移指南](https://github.com/lin-snow/Ech0/blob/main/docs/usage/storage-migration.md)**。
