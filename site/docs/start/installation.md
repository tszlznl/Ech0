...
除 `JWT_SECRET` 外，其余变量（数据库路径、日志级别、对象存储等）以仓库 **`README.zh.md`** 与 **`docs/`** 目录说明为准。时区语义请参阅 `docs/dev/timezone-design.md`：站点级统计固定 UTC，用户视角接口（如「今日」、热力图）继续使用浏览器请求的 **`X-Timezone`**。生产环境建议：
