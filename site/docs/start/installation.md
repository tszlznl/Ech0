---
title: 安装部署
description: Docker / Compose / 二进制 / Helm；端口、数据、时区与安全要点
---
...
| `TZ`（可选） | IANA 时区名（如 `Asia/Shanghai`、`America/New_York`）。影响服务端进程本地时间，以及**无用户上下文**时的按日统计；浏览器里「今日」等仍以客户端时区为准。 |
...
- **`TZ`**：建议设为实例所在地区或你期望的 IANA 时区；不配则多为 UTC。与仓库根目录 `README.zh.md` 中「极速部署」一致。
...
services:
  ech0:
    ...
    environment:
      - JWT_SECRET=请改为随机强密码
      # 可选：IANA 时区，与 README / 仓库 docker-compose.yml 中说明一致
      - TZ=Asia/Shanghai
...
- 需要与容器部署相同的「进程本地时区」时，可在 **systemd** 单元或环境中设置 **`TZ`**（IANA 名称，如 `Asia/Shanghai`），与上文 Docker **`TZ`** 语义一致。
...
可选：通过 `extraEnv` 注入 `TZ`（与 Docker 中 `-e TZ=…` 含义相同），例如：
...
```bash
export TZ=Asia/Shanghai   # 可选，与容器内 TZ 语义一致；也可写入 systemd 单元 Environment=TZ=…
./ech0 serve
```

常见架构包括 `linux/amd64`、`linux/arm64`、`linux/armv7`、`windows/amd64` 等。

Windows 可在「系统环境变量」或启动前执行 `set TZ=Asia/Shanghai`（以你使用的 shell 为准）。
...
除 `JWT_SECRET` 外，其余变量（数据库路径、日志级别、对象存储等）以仓库 **`README.zh.md`** 与 **`docs/`** 目录说明为准。**时区**：部署时设置 **`TZ`**（IANA 名称）可让服务端 `time.Local` 与日志、站点级按日统计等与你的地区一致；用户视角的「今日」、热力图等接口仍使用浏览器请求的 **`X-Timezone`**，二者职责不同，详见仓库内 `docs/dev/timezone-design.md`。生产环境建议：

- 不要用默认示例密钥；
- 按需设置 **`TZ`**，避免服务器默认 UTC 与直觉不符；
- 定期备份数据目录与快照；
